...
async function discover() {
  ...
  return Array.from(new Map(found.map(item => [item.url || item.title, item])).values()).sort((a,b)=>b.score-a.score).slice(0, 8);
}
...
async function enrich(items) {
  if (!process.env.GEMINI_API_KEY || !items.length) return fallback(items);
...
try {
  ...
  const items = await enrich(discovered);
  ...
  await writeFile(path, `${JSON.stringify({ lastChecked, generatedBy: process.env.GEMINI_API_KEY ? "Gemini-assisted official-source monitor" : "Rules-based official-source monitor", items }, null, 2)}\n`);
